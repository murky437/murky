...
function Sidebar() {
  ...
  const { initialProjects } = useLoaderData({ from: '/_auth/notes' }) as {
  ...
  const [settingsContextMenu, setSettingsContextMenu] = useState<{ x: number; y: number } | null>(
    null
  );
  ...
  return (
    <>
      <div className={styles.wrapper} onContextMenu={e => openSidebarContextMenu(e)}>
        ...
        <div>
          <div className={styles.settings} onClick={e => openSettingsContextMenu(e)}>
            ☰
          </div>
        </div>
        {settingsContextMenu && (
      ...
      {isAddOpen && <AddProjectModal onClose={closeAddModal} onSuccess={refreshProjects} />}
    </>
  );
}
